...
class Sobs():
	def __init__(self, n_thread=14, vwidthkms=3, dvkms=0.04, sizeau=args.dpc*15, dxasec=0.08, npixy=1, rect_camera=True, omp=True):
		self.dpc = 137
		self.iline = 1
		self.n_thread = n_thread
		self.vwidthkms = vwidthkms
		self.sizeau = sizeau

	def mctherm():
		tools.exe('cd calc/radmc ; radmc3d mctherm setthreads 16')
...
class ObsData:
	...
	def make_fits_data(self, n_thread=14, widthkms=3, dvkms=0.04, sizeau=args.dpc*15, dxasec=0.08, npixy=1, rect_camera=True, omp=True):
		...
		if omp:
			...
			rets = Pool(n_thread).map(_subcalc, [(p, 'proc' + str(p), cmd(p)) for p in range(n_thread)])
			data = rets[0]
			for ret in rets[1:]:

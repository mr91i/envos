...
param_name = "bs0.125"
param_dict = {"bs0.125": ["./data9-2", "CM_bs0.125_T30_CR*_M*_cav45"],
              }[param_name]
...
def data_from_pvcor(ranking=True):
    dplist_PV2 = [path for path in glob.glob(data_dir+"/fig_*") if pat_PV2.search(path)]
    ...
    if ranking:
        ...
        for i, pvd in enumerate(pvd_list_sorted[:10]):
            print(f"{i+1}: CR={pvd.CR} , M={pvd.M} -->  {pvd.corr}")
    return pvd_list
...
def gen_pvd(filename, mode="fits"):
    if mode =="fits":
        with fits.open(filename) as hdulist:
            hdu = hdulist[0]
            pvd = PVdiagram(copy.copy(hdu.data), hdu.header, x1style="au", x2style="km/s")
            del hdu.data
        return pvd
def make_pvdlist_corr(pvd1, dp_list):
    def gen_data(dp):
        ...
        pvd2.M, pvd2.CR = get_param_from_inputfile(dp)
...
def calc_ZNCC(im1, im2):
    ...
    ZNCC_AB = (imax1*jmax1*sumAB - sumA * sumB)/ \
               np.sqrt(imax1*jmax1*sumAsq - sumA**2)/np.sqrt(imax1*jmax1*sumBsq - sumB**2)
    return ZNCC_AB
...
def make_pvd_list(dp_list):
    def gen_data(dp):
        ...
        pvd.M, pvd.CR = get_param_from_inputfile(dp)
...
def calc_Mass(pvd, threshold=0, f_crit=0.2, method="vpeak"):
    ...
    jmax, imax = im.shape
    if method=="vpeak":
        x_vmax, I_vmax = np.array([[ get_maximum_position(pvd.xau, Iv), np.max(Iv) ] for Iv in im.transpose(0, 1)]).T
...
def read_data_from_PVchi2():
    PV2_pathlist_out = [path for path in glob.glob(f"{out_dir}/*.out") if pat_PV2.search(path)]
...
def plot_map_SSD(table):
    ...
    mp.map(points=points, mode=plotmode, xlim=[0.8,3.8], ylim=[-2.2,1.2], square=True, save=False,
           cbl="", xl="log CR [au]", yl=r"log M [M$_{\odot}$]")
    mp.ax.scatter(np.log10(CR0), np.log10(M0), c="r", s=50 , alpha=1, linewidth=0, zorder=10)
    ibest = np.argmin(points[:,2])
    mp.ax.scatter(points[ibest,0], points[ibest,1], c="orange", s=30 , alpha=1, linewidth=0, zorder=10)
    zfun_rbf = interpolate.Rbf(points[:,0], points[:,1], points[:,2], function='cubic', smooth=0)  # default smooth=0 for interpolation
    res = optimize.minimize(lambda v: zfun_rbf(v[0],v[1]), np.array([np.log10(CR0), np.log10(M0) ]) )
    mp.ax.scatter(res.x[0], res.x[1], c="green", s=30 , alpha=1, linewidth=0, zorder=10)
    mp.save("map")

def plot_map_ZNCC(pvd_list):
    table = np.array([[pvd.CR, pvd.M, pvd.corr] for pvd in pvd_list ])
    ...
    print(points)
    mp.map(points=points, mode=plotmode, xlim=[1.3,3.3], ylim=[-1.7,0.3], square=True, save=False,
           cbl="", xl="log CR [au]", yl=r"log M [M$_{\odot}$]", cbax=cbax)
    mp.ax.scatter(np.log10(CR0), np.log10(M0), c="r", s=50 , alpha=1, linewidth=0, zorder=10)
    ibest = np.argmax(points[:,2])
    # mp.ax.scatter(points[ibest,0], points[ibest,1], c="orange", s=30 , alpha=1, linewidth=0, zorder=10)
    points_ip = points[~np.isnan(points).any(axis=1)]
    zfun_rbf = interpolate.Rbf(points_ip[:,0], points_ip[:,1], -points_ip[:,2], function='cubic', smooth=0)  # default smooth=0 for interpolation
    res = optimize.minimize(lambda v: zfun_rbf(v[0],v[1]), np.array([np.log10(CR0), np.log10(M0) ]) )
    ...
    mp.ax.scatter(points_false[:,0], points_false[:,1], c="red", s=30, marker="x", alpha=1, linewidth=1, zorder=10)
    mp.save("map")

def plot_map_Mest(pvd_list, tag=""):
    table = np.array([[pvd.CR, pvd.M, pvd.Mest] for pvd in pvd_list ])
    table_nonan = table[ ~np.isnan(table[:,-1]) ]
    points  = np.array(table_nonan)
    ...
    cnlim=[-60,60]
    ctdelta = 10
    ctax = np.concatenate([ np.arange(-ctdelta/2, max(min(points[:,2]), cnlim[0])-ctdelta , -ctdelta)[::-1],
            np.arange(ctdelta/2, min(max(points[:,2]), cnlim[1])+ctdelta , ctdelta) ])
    mp.map(points=points, mode=plotmode, square=True, save=False, logxy=True,
            cbl="Error in Estimated Mass [%]", xl="CR [au]", yl=r"M [M$_{\odot}$]",
            ctax=ctax, cmap=plt.cm.get_cmap("coolwarm"), cnlim=cnlim)
    ...
    mp.map(points=points_new, mode=plotmode,out="mapM", logxy=True, square=True,
           cnlim=cnlim, cmap=plt.cm.get_cmap("coolwarm"),
            cbl="Error in Estimated Mass [%]", xl="CR [au]", yl=r"M [M$_{\odot}$]")

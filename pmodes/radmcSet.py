...
class SetRadmc:
    def __init__(self, dirpath_radmc,
                 ...
                 tgas='tdust', line=True, turb=True, lowreso=False, debug=False, subgrid=False, auto=True, fn_model_pkl="res_L1527.pkl"):
        ...
        self.fn_model_pkl = fn_model_pkl
    ...
    def read_model_data(self):
        D = pd.read_pickle(self.dpath+self.fn_model_pkl)
        self.D = namedtuple("Model", D.keys())(*D.values())

    def set_grid(self):
        if self.subgrid:
            nr, ntheta, nphi = [128, 64, 1] if self.lowreso else [512, 256, 1]
            #r_range = [self.rin, self.rout]
            #theta_range = [  ]
            #rin = 1 * cst.au
            #rout = 1000 * cst.au
            #thetaup = 0.0 / 180.0 * np.pi
            ri = np.logspace(np.log10(self.rin), np.log10(self.rout), nr+1)
            thetai = np.linspace(self.thetain, 0.5*np.pi, ntheta+1)
            #thetai1 = np.linspace(thetain, 0.25*np.pi, ntheta/4.+1)
...
#   ret = np.nan_to_num(ret)
    if logz:
        ...
        # np.where( sgn!=0, np.sign(sgn)*10**ret, 0 )

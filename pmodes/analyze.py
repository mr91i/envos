...
#        xx, yy = np.meshgrid(self.xau, self.yau)
    ...
    def pvdiagram(self, n_lv=50):
        ...
        pltr.map(z=Ipv, mode=self.plotmode_PV,
        ...
        #plt.plot(self.xau/self.dpc, l/(self.xau*cst.au)/cst.kms, c="green", ls=":", lw=1)
        #plt.plot(self.xau/self.dpc, l/(self.xau*cst.au)/cst.kms*(a)**(0.5), c="navy", ls=":", lw=1)
        #plt.plot(self.xau/self.dpc, l/(self.xau*cst.au)/cst.kms*(a), c="plum", ls=":", lw=1)
        #plt.plot(self.xau/self.dpc, -l/(self.xau*cst.au)/cst.kms, c="green", ls=":", lw=1)
        #plt.plot(self.xau/self.dpc, -l/(self.xau*cst.au)/cst.kms*(a)**(0.5), c="navy", ls=":", lw=1)
        #plt.plot(self.xau/self.dpc, -l/(self.xau*cst.au)/cst.kms*(a), c="plum", ls=":", lw=1)
        plt.plot(self.xau/self.dpc, 2*l/rCR*(2*self.xau*cst.au/rCR)**(-2/3)/cst.kms, c="hotpink", ls=":", lw=1)
        #plt.plot(self.xau/self.dpc, np.sqrt(2*cst.G*self.Mstar*cst.Msun/(self.xau*cst.au))*np.sqrt(2)/3**(3/4)/cst.kms, c="hotpink", ls=":", lw=1)

        if self.oplot_KeplerRotation:
            plt.plot(self.xau/self.dpc, np.sqrt(cst.G*self.Mstar*cst.Msun/ \
        ...
        if self.oplot_LocalPeak_Vax:
            for Iv, v_ in zip(Ipv.transpose(0, 1), self.vkms):
                for xM in _get_peaks(self.xau, Iv):
                    plt.plot(xM/self.dpc, v_, c="red", markersize=1, marker='o')

        if self.oplot_LocalPeak_Pax:
            for Ip, x_ in zip(Ipv.transpose(1, 0), self.xau):
                for vM in _get_peaks(self.vkms, Ip):
                    plt.plot(x_/self.dpc, vM, c="blue", markersize=1, marker='o')

        if self.oplot_LocalPeak_2D:
            for jM, iM in peak_local_max(Ipv, min_distance=10):
                plt.scatter(self.xau[iM]/self.dpc, self.vkms[jM],
                            c="k", s=20, zorder=10)

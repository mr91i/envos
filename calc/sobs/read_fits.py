...
print("fitsfilenam : ", fitsfilename )
print("beam_size[au]:",beam_size)
print("pixel size[au]:",dx,dy)
print("L[au]:",Lx,Ly)
...
#	print(data.shape, v_width/dv , np.abs( beam_size/dy ) ,   np.abs( beam_size/dx ) )
	std = np.array([ v_width/dv , np.abs( beam_size/dy ) , np.abs( beam_size/dx )] )/2.354820
	data = gaussian_filter(data, sigma=std )

	data *= np.abs(dx*dy) /( np.pi/4*0.9*0.4 )
	data *= dv/v_width
...
#	im = Contour(xx, vv, I_pv , n_lv=n_lv , mode="contour", cmap=plt.get_cmap('Greys'),cbmin=0)
	...
	def get_peaks( x , y ):
		maxis = []
		for mi in peak_local_max(y, min_distance=1)[:,0] :
	...
	if mark_peak:
		if mark_peak[0]:
			for I_v, av in zip( I_pv.transpose(0,1) , vkms ):
				for xM in get_peaks(x,I_v):
					plt.plot(  xM , av , c="red", markersize=1 , marker='o')

		if mark_peak[1]:
			for I_p, ax in zip( I_pv.transpose(1,0) , x ):
				for vM in get_peaks(vkms,I_p):
					plt.plot(  ax , vM , c="blue", markersize=1 , marker='o')

	for jM, iM in peak_local_max(I_pv, min_distance=3):

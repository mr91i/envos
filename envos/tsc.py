...
class TscData:
    ...
    def variables(self):
        return (self.alpha_0, self.alpha_M, self.alpha_Q, self.V_0, self.V_M, self.V_Q, self.W_Q, self.m_0, self.da0dy, self.dV0dy)
...
class TscSolver:
    ...
    def solve_0thorder(self):
        ...
        def func0th(y, v):
            V_0, al_0 = v
            dV_0 =      (al_0*(y - V_0) - 2/y)*(y-V_0)/((y-V_0)**2 -1) # if x < 1 else 0
            dal_0 = al_0*(al_0 - 2/y*(y - V_0))*(y-V_0)/((y-V_0)**2 -1) #if x < 1 else -4/x**3
        ...
        def jacfunc(y, v):
            ...
            f1y2 = s**2/(s**2-1)
            f2y1 =  2*al0*s**2*(al0 - 2 +2*V0/y)/(s**2-1)**2  + (4*al0 -4*al0*V0/y -al0**2)/(s**2-1)
            f2y2 = s*(2*al0 -2 +2*V0/y)/(s**2-1)
            return np.array([[f1y1, f1y2], [f2y1, f2y2]])

        eps = 2*self.eps
        sol = integrate.solve_ivp(func0th, (self.xin[0], self.xin[-1]),
                                  [-eps, 2+2*eps], t_eval=self.xin,
                                  jac=jacfunc, method='BDF', vectorized=True, rtol=self.eps, atol=1e-10)
        ...
        if len(self.xin) != len(sol.t):
            raise Exception(f"x finally used in the solution is not equal to the input x. \n input x: {self.xin.shape}, used x: {sol.t.shape}")
    ...
    def f_dal0dy(self, y):
        al_0 = self.f_al0(y)
        V_0 = self.f_V0(y)
        return np.where((y-V_0)**2 != 1, al_0*(al_0 - 2/y*(y - V_0))*(y-V_0)/( (y-V_0)**2 -1 ), -2/y**2)

    def f_dV0dy(self, y):
        al_0 = self.f_al0(y)
        V_0 = self.f_V0(y)
        return np.where((y-V_0)**2 != 1, (al_0*(y - V_0) - 2/y)*(y-V_0)/((y-V_0)**2 -1), 1)
    ...
    def solve_Qualdrapolar(self, search_K=True):
        ...
        def sol_Q(K):
            opt = {"method":{0:'RK45', 1:'BDF', 2:'Radau', 3:'DOP853', 4:"LSODA"}[1], "rtol":self.eps, "atol":1e-10}
            x0 = self.x[0]
            y0 = (-2/7*K*x0**(-7), -K/2*x0**(-4), K/6*x0**(-4), K*(1+x0**(-2)/35), -2/245*K*x0**(-7))
            sol_o = integrate.solve_ivp(f_QuadraPolar_out, (self.xout[0], self.xout[-1]), y0, t_eval=self.xout, jac=fjac_QuadraPolar_out, **opt)
        ...
        def loop_K(K):
            ...
            if self.plot:
                ...
                plt.ylim(-5, 7)
                logger.info(f"{j}: K={K}")
                plt.savefig(f"{config.dp_fig}/alpha_{j}.pdf")
    ...
    def solve_Monopolar(self):
        ...
        y0 = (1/2, 0, 0)
        solM = integrate.solve_ivp(f_MonoPolar, (self.x[0], self.x[-1]), y0, t_eval=self.x, method='BDF', rtol=1e-8)
        self.al_M = solM.y[0]
        self.V_M = solM.y[1]
        self.M = -integrate.simps(self.x[self.x<1]**2 * (self.al_M[self.x<1] - 1/2), self.x[self.x<1])

    #def save_table(self, filename="tscsol.dat", path=None):
    #    soldict = asdict(self.get_solution())
    #    path = path or os.path.join(config.dp_storage, filename)
    #    vals = np.array(list(soldict.values())).T
    #    np.savetxt(path, vals, header=" ".join(soldict.keys())  )
    def print_result_constants(self):
        logger.info(f"result: K={self.K}, ΔQ={self.Delta_Q}, m*={self.M}")
        print(f"result: K={self.K}, ΔQ={self.Delta_Q}, m*={self.M}")
    ...
    def get_solution(self):
        sol = TscData(self.x, self.al_0, self.al_M, self.al_Q, self.V_0, self.V_M, self.V_Q, self.W_Q, self.m_0, self.da0dy, self.dV0dy, self.Delta_Q)
        return sol
...
if __name__=="__main__":
    ...
    plt.xlim(1e-3, 1e1)
    plt.ylim(1e-5, 1e7)
    l = plt.plot(sol.x, np.array([sol.alpha_0, sol.alpha_M, sol.alpha_Q, -sol.alpha_Q]).T)
    plt.savefig(f"{config.dp_fig}/tscfig3.pdf")
    [_l.remove() for _l in l]
    plt.xlim(1e-3, 1e0)
    plt.ylim(1e-4, 1e3)
    l = plt.plot(sol.x, np.array([-sol.V_0, sol.V_M]).T)
    plt.savefig(f"{config.dp_fig}/tscfig4a.pdf")
    [_l.remove() for _l in l]
    plt.xlim(1e-3, 1e1)
    plt.ylim(1e-8, 1e3)
    l = plt.plot(sol.x, np.array([-sol.V_Q, sol.V_Q, -sol.W_Q]).T )
    plt.savefig(f"{config.dp_fig}/tscfig4b.pdf")

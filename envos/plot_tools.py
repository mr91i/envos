...
def plot_midplane_velocity_profile(model, rlim=400, ylim=(-0.5, 4)):
    ...
    plt.plot(
        model.rc_ax / nc.au,
        -model.vr[:, index_mid, 0] * cav / 1e5,
        label=r"$- v_r$",
        ls="-",
    )
    plt.plot(
        model.rc_ax / nc.au,
        model.vt[:, index_mid, 0] * cav / 1e5,
        label=r"$v_{\theta}$",
        ls=":",
    )
    plt.plot(
        model.rc_ax / nc.au,
        model.vp[:, index_mid, 0] * cav / 1e5,
        label=r"$v_{\phi}$",
        ls="--",
    )
...
def plot_temperature_map(
    ...
    T = m.Tgas
    lvs = np.arange(
        10 * np.floor(1 / 10 * np.min(T)), np.max(T[m.rr > 50 * nc.au]) + 10 + 1e-10, 10
    )

...
class RadmcController:
    ...
    def __init__(self, model=None, dpath_run=None, dpath_radmc=None, dpath_storage=None):
        ...
        self.set_dir(radmc=self.dpath_radmc, run=self.dpath_run, storage=self.dpath_storage)
...
class ThermalKinematicModel:
    def __init__(self, dpath_radmc=None, ispec=None, f_dg=None, tgas_eq_tdust=True):
        ...
        self.R = self.rrc * np.sin(self.ttc)
        self.z = self.rrc * np.cos(self.ttc)
...
class Streamline:
    ...
    def save_data(self, filename="stream", filepath=None):
        ...
        np.savetxt(f'{dpath}/{filename}_{self.name}.txt', stream_data, header=header)
